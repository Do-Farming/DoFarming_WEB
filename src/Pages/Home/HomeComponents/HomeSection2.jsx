    ...
    const fetchPackages = async () => {
      try {
        ...
        const newPackages = response.data.map((pkg) => {
          const [routine] = pkg.content.split(', '); // 수정: content를 분리하여 루틴 이름만 추출
          return { ...pkg, routine }; // 새로운 패키지 객체를 생성하여 routine 프로퍼티를 추가
        });
        setPackages(newPackages); // 서버에서 받은 패키지 목록을 상태에 설정
      } catch (error) {
            ...
            <div key={pkg.trackId}>
              <div className="S2Wrap">
                <div className="S2Wrap2">
                  <div id="userRname">{pkg.routine}</div> {/* 수정: 루틴 이름 표시 */}
                  <button onClick={(e) => {e.stopPropagation(); navigate('/HomeEditPackage');}} className="BtnS2">
